...
const WhatWeDoComponent = () => {
  ...
  const getData = useCallback(async (id) => {
    const res = await dispatch(
      await postsWhatWeDoDetailApi.endpoints.whatDetail.initiate(id)
    );
        ...
        <div className={styles.content}>
          <div className={styles.bottomBlock}>
            <h1 className={styles.h1Title}>
              {postWhatWeDoDetail?.html_h1_tag}
            </h1>
            <div
              className={styles.blockItemImage}
              dangerouslySetInnerHTML={{
                __html: postWhatWeDoDetail?.create_page || "",
              }}
            />
            <Paragraph className={styles.title}>Explore more</Paragraph>
            <Row className={styles.blockItems}>
              {postsWhatWeDoApi?.data_list &&
                getRandomValuesFromArray(postsWhatWeDoApi?.data_list, 3).map(
                  (el, i) => (
                    <Link
                      ...
                      prefetch={false}
                    >
                      <WeDoCard item={el} fromDetail={true} />
                    </Link>
                  )
                )}
            </Row>
